...
#         module_idx = 0
#         if 'ACM' in module:
#             self.features.add_module(module+str(module_idx), ACM(32, in_channels))
#         elif 'CBAM' in module:
#             self.features.add_module(module+str(module_idx), CBAM(in_channels,in_channels))
#         elif 'DEEPRFT' in module:
#             self.features.add_module(module+str(module_idx), DeepRFT(in_channels,in_channels))
#         elif 'FFC' in module:
#             self.features.add_module(module+str(module_idx), FFC_BN_ACT(in_channels,in_channels,norm_layer=norm))
#         # elif 'MHA':
#         #     featureLength = 1280
...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("norm0", get_norm_layer(name=norm, spatial_dims=spatial_dims, channels=init_features)),
                    ("relu0", get_act_layer(name=act)),
                    # ("pool0", pool_type(kernel_size=3, stride=2, padding=1)), ## originally here
                ]
            )
        )
        ...
        self.extract_stacks.append(len(self.features))
        self.features.add_module("pool0", pool_type(kernel_size=3, stride=2, padding=1)),
